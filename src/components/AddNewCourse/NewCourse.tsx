...
const NewCourse: React.FC = () => {
  ...
  const [uploadProgress, setUploadProgress] = useState<{
    [key: string]: { video?: number; doc?: number; txt?: number };
  }>({});
    ...
    try {
      if (lectureIndex !== undefined) {
        setLectures((prev) =>
          prev.map((lec, i) =>
            i === lectureIndex
              ? { ...lec, uploading: true, uploadError: "" }
              : lec
          )
        );
      ...
      if (lectureIndex !== undefined) {
        setLectures((prev) =>
          prev.map((lec, i) =>
            i === lectureIndex
              ? { ...lec, uploading: false, uploadError: "" }
              : lec
          )
        );
        setUploadProgress((prev) => ({
      ...
      if (lectureIndex !== undefined) {
        setLectures((prev) =>
          prev.map((lec, i) =>
            i === lectureIndex
              ? { ...lec, uploading: false, uploadError: `فشل رفع ${type}` }
              : lec
          )
        );
        setUploadProgress((prev) => ({
  ...
  const handleCreate = async () => {
    const effectiveTeacherName =
      user && user.name ? user.displayName : teacherName;
    const effectiveTeacherId = user && user.uid ? user.uid : "guest-user";
    ...
    for (let i = 0; i < lectures.length; i++) {
      ...
      if (lec.uploading) {
        toast.info(
          `يرجى الانتظار حتى انتهاء رفع الملفات للمحاضرة رقم ${i + 1}`
        );
        return;
      }
      if (lec.uploadError) {
        toast.error(
          `يوجد خطأ في رفع ملفات المحاضرة رقم ${i + 1}: ${lec.uploadError}`
        );
        return;
      }
    }
      ...
      >
        <Box sx={{ mt: 2, width: "100%", borderRadius: 2, mx: "auto" }}>
          ...
          >
            ...
            <Button
              onClick={() => navigate("/teacherdashboard/courses")}
              sx={{ minWidth: "auto" }}
            >
              <CloseIcon />
            </Button>
                        ...
                        <ListItem>
                          <Stack direction={"column"} spacing={1}>
                            <Box sx={{ m: 2, ml: 17 }}>
                              ...
                              {uploadProgress[index]?.video !== undefined &&
                                lecture.uploading && (
                                  <Box sx={{ width: "80%", mt: 1 }}>
                                    <LinearProgress
                                      variant="determinate"
                                      value={uploadProgress[index]?.video || 0}
                                    />
                        ...
                        <ListItem>
                          <Stack direction="column" spacing={2}>
                            <Box>
                              ...
                              {uploadProgress[index]?.doc !== undefined &&
                                lecture.uploading && (
                                  <Box sx={{ width: "80%", mt: 1 }}>
                                    <LinearProgress
                                      variant="determinate"
                                      value={uploadProgress[index]?.doc || 0}
                                    />
                                    <Typography variant="caption">
                                      {uploadProgress[index]?.doc || 0}%
                                    </Typography>
                                  </Box>
                                )}
                              {lecture.docUrl && (
                                <Typography color="success.main">
                                  تم رفع ملف DOC بنجاح
                                </Typography>
                              )}
                            </Box>
                          </Stack>
                        ...
                        <ListItem>
                          <Stack direction="column" spacing={2}>
                            <Box>
                              ...
                              {uploadProgress[index]?.txt !== undefined &&
                                lecture.uploading && (
                                  <Box sx={{ width: "80%", mt: 1 }}>
                                    <LinearProgress
                                      variant="determinate"
                                      value={uploadProgress[index]?.txt || 0}
                                    />
                                    <Typography variant="caption">
                                      {uploadProgress[index]?.txt || 0}%
                                    </Typography>
                                  </Box>
                                )}
                              {lecture.txtUrl && (
